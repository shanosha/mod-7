...
form.addEventListener("submit", async (e) => {
  ...
  if (searchInput.value == "") {
    ...
    searchInput.value != "" &&
    getHostType(searchInput.value) != "invalid"
  ) {
    searchInput.setCustomValidity("");

    try {
      ipAddress = searchInput.value;
      data = await fetchGeoLocation(
        ipAddress,
        getHostType(searchInput.value) == "domain",
      );
      if (data === undefined) {
      ...
      marker.setPopupContent(
        `<b>New Location</b><br>${data.location.city}, ${data.location.country}`,
      );
